...
def lookup(symbol):
    ...
    # Contact API
    try:
        api_key = os.environ.get('IEX_CLOUD_key')
...
def company_info(symbol):
    ...
    try:
        api_key = os.environ.get('IEX_CLOUD_key')
        response = requests.get(
            f"https://cloud-sse.iexapis.com/stable/stock/{urllib.parse.quote_plus(symbol)}/company?token={api_key}")
        response.raise_for_status()
    except requests.RequestException:
        return None
...
def company_stats(symbol):
    ...
    try:
        api_key = os.environ.get('IEX_CLOUD_key')
        response = requests.get(
            f"https://cloud-sse.iexapis.com/stable/stock/{urllib.parse.quote_plus(symbol)}/stats?token={api_key}")
        response.raise_for_status()
    except requests.RequestException:
        return None
...
def dividend(symbol):
    ...
    try:
        api_key = os.environ.get('IEX_CLOUD_key')
        response = requests.get(
            f"https://cloud-sse.iexapis.com/stable/stock/{urllib.parse.quote_plus(symbol)}/dividends/next?token={api_key}")
        response.raise_for_status()
    except requests.RequestException:
        return None
    ...
    try:
        ...
        if quote["amount"] is None:
            quote["amount"] = 0.0
        elif quote["exDate"] is None:
            quote["exDate"] = "None"
        elif quote["paymentDate"] is None:
...
def company_info2(symbol):
    ...
    try:
        ...
        response = requests.get(
            f"https://finnhub.io/api/v1/stock/profile2?symbol={urllib.parse.quote_plus(symbol)}&token={api_key}")
        response.raise_for_status()
    except requests.RequestException:
        return None
...
def company_peers(symbol):
    ...
    try:
        api_key = os.environ.get('FINNHUB_key')
        response = requests.get(
            f"https://finnhub.io/api/v1/stock/peers?symbol={urllib.parse.quote_plus(symbol)}&token={api_key}")
...
def create_sunburst_plots(users_stocks):
    ...
    current_diversification = pd.DataFrame(list(zip(indx, sector, name, price)),
                                           columns=['', 'sector', 'name', 'value'])
    ...
    df_current_diversification = build_hierarchical_dataframe(current_diversification, levels, value_column,
                                                              color_columns)

    fig = make_subplots(1, 2, specs=[[{"type": "domain"}, {"type": "domain"}]],
                        subplot_titles=('Initial Allocation:', 'Current Allocation:'))
...
def create_portfolio_indicator_plots(users_stocks, port_yield):
    ...
    gauges_data = pd.DataFrame(list(zip(indx, sect, name, value, price)),
                               columns=['', 'sector', 'name', 'value', 'price'])
    ...
    fig.update_layout(height=150, plot_bgcolor='#ffffff', paper_bgcolor='#ffffff',
...
def create_dividend_plot(users_stocks):
    ...
    fig.update_layout(height=250, plot_bgcolor='#ffffff', paper_bgcolor='#ffffff',
                      margin=dict(l=10, r=10, b=0, t=15, pad=4))
