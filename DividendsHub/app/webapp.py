...
from app.models import User, Stocks, Deposits, Dividends
...
def index():
    ...
    if current_user.is_authenticated:
        ...
        if users_stocks:
            ...
            return render_template("index.html", title='Home Page', stocks=stocks, total_cost_basis=round(total_cost_basis,2), total_market_value=round(total_market_value,2), plot=plot)
    
    return render_template("index.html", title='Home Page', stocks=stocks, total_cost_basis=round(total_cost_basis,2), total_market_value=round(total_market_value,2))
    ...
    if current_user:
        ...
        if form.validate_on_submit():
            ...
            if symbol:
                ...
                new_cost_basis = round((float(symbol.amount_of_shares)*float(symbol.cost_basis) + float(form.number_of_shares.data)*float(form.cost_basis.data))/(new_amount_of_shares),2)
                ...
                new_stock = Stocks(symbol=ticker, name=lookup(ticker)['Name'], amount_of_shares=form.number_of_shares.data, cost_basis=form.cost_basis.data, sector= sector, owner=current_user)
            ...
            if form1.portfolio.data:
                ...
                for index, row in df.iterrows():
                    ...
                    if lookup(ticker) == None:
                        ...
                        new_stock = Stocks(symbol=ticker, name=lookup(ticker)['Name'], amount_of_shares=float(row['Quantity']), cost_basis=float(row['Cost Per Share']), sector= Equity(ticker).sector, owner=current_user)
                        db.session.add(new_stock)
                        db.session.commit()
                    else:
                        new_amount_of_shares = round(float(stock.amount_of_shares) + float(row['Quantity']),2)
                        new_cost_basis = round((float(stock.amount_of_shares)*float(stock.cost_basis) + float(row['Quantity'])*float(row['Cost Per Share']))/(new_amount_of_shares),2)
        ...
        if users_stocks:
            for index, row in enumerate(users_stocks):
                ...
                if not dividend_info:
                    ...
                    div.append(list((row.symbol, row.name, dividend_info['exDividendDate'], 'Feature coming soon', round(dividend_info['ttmDividend'],2), round(float(dividend_info['ttmDividend'])*row.amount_of_shares,2))))
